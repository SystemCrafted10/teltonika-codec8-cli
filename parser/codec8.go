...
func ParseCodec8(data []byte) (*TeltonikaAvlPacket, error) {
	...
	for i := 0; i < int(packet.Quantity1); i++ {
		...
		for j := 0; j < n2; j++ {
			...
			id := int(data[offset])
			val := int(bytesToUint16(data[offset+1 : offset+3]))
			ioElem.Elements[fmt.Sprintf("%d", id)] = val
			offset += 3
		}
		...
		for j := 0; j < n4; j++ {
			...
			id := int(data[offset])
			val := int32(bytesToUint32(data[offset+1 : offset+5]))
			ioElem.Elements[fmt.Sprintf("%d", id)] = val
			offset += 5
		}
		...
		for j := 0; j < n8; j++ {
			...
			id := int(data[offset])
			val := bytesToUint64(data[offset+1 : offset+9])
